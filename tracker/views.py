...
def send_invitation(request, group_id, group=None):
    ...
    if emails:
        if group.owner.id == current_user.id:
            ...
            messages.error(request, "You don't have permission to send invitation")

    return redirect(
        request.META.get("HTTP_REFERER", reverse("group_view", args=[group_id]))
    )
...
def ticket_list(request):
    ...
    return render(request, "tickets/ticket_list.html", {"user_tickets": user_tickets})
